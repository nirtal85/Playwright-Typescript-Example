...
export class SftpService {
	 ...
	 */
	async countFilesWithPrefix(
		filePrefix: string,
		remoteDir = "/remote/inbox",
	): Promise<number> {
		return await this.performSftpAction(async (client) => {
			const fileList = await client.list(remoteDir);
			return fileList.filter(
				(file) => file.type === "-" && file.name.startsWith(`${filePrefix}-`),
			).length;
		});
	}
	 ...
	 */
	private async performSftpAction<T>(
